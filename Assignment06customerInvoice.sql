...
-- 1. Show every product (description only) with a price of $10.00 or more, along with its price. (output = 1 column)
select product.p_descript from product where product.p_price > 10;

-- 2. Show all invoice numbers belonging to customer Leona Dunne (output = 1 column)
select invoice.inv_number from invoice,customer
where customer.cus_code = invoice.cus_code and customer.cus_fname = 'Leona' and customer.cus_lname='Dunne'
;

-- 3. Show every product (description only) related to saws (LIKE '%saw%') along with the supplying vendor (name only). (requires join, output = 2 columns)
select product.p_descript from product,vendor
where 
select *
from 
...
from product
join vendor
on vendor.v_code = product.v_code) as myproducts
where myproducts.p_descript like '%saw%'
;

-- 4. Show every product (description only) purchased by Kathy Smith. (requires join, output = 1 column)
...
where customer.cus_fname='Kathy' and customer.cus_lname='Smith'
;
...
alter table vendor
add column ISO9000 char(1) default 'N' not null;


-- 6. Use SELECT to list vendor names and product descriptions, matching products with their vendors using keys. Use a FULL OUTER JOIN so that unmatched vendors and products will still be in the output. (output = 2 columns)
select vendor.v_name, product.p_descript
from vendor
full outer join product
on vendor.v_code = product.v_code; 

-- 7. Use SELECT to create a simple list of invoices belonging to those customers in the '615' area code. You will need to join customer to invoice on two criteria: matching cus_code AND cus_areacode matching the string above. (output = 1 column)
...
-- 8. Use aggregate functions and math operations to total up the amount of money spent by every single individual in the database. You will need to join customer with invoice and line and output first and last names with dollar amounts. (output = 3 columns)
select customer.cus_fname, customer.cus_lname, sum(line.line_units * line.line_price)
from customer
join invoice
on invoice.cus_code = customer.cus_code
join line
...
-- 10. Use DELETE and a subquery to remove all customers from the database if they have no invoices. You can utilize the query above as a subquery.
...
-- select * from customer;   		used to test if delete statement worked
